...
import { Upload, X, FileText, Image as ImageIcon, File, Trash2, ExternalLink } from "lucide-react";
...
interface Anexo {
  ...
  tamanho: number;
  createdAt: Date;
  publicUrl?: string; // resolvida em client ou via API
}
...
// Gera URL de acesso a partir do src supabase://
function getDownloadUrl(url: string, nome: string, inline = false) {
  return `/api/files/anexo?src=${encodeURIComponent(url)}&filename=${encodeURIComponent(nome)}&inline=${inline ? "1" : "0"}`;
}
      ...
      {canUpload && (
        <>
          ...
          >
            ...
            <p className="text-xs text-gray-400">Imagens e PDF — máx 10 MB</p>
          </div>
      ...
      {anexos.length === 0 ? (
        ...
        <div className="divide-y divide-gray-50">
          {anexos.map((a) => {
            ...
            const viewUrl = getDownloadUrl(a.url, a.nome, isImg); // inline para imagens
            const dlUrl = getDownloadUrl(a.url, a.nome, false);
              ...
              <div key={a.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors group">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <a href={viewUrl} target="_blank" rel="noopener noreferrer"
                    className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors" title="Abrir">
