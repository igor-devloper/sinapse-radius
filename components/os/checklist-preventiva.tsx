                  ...
                  <div key={item.id} className={cn("transition-colors", s.bg)}>
                    <div className="px-5 py-3 flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {item.asset?.nome && (
                          <div className="mt-2 flex items-center gap-3 rounded-xl border border-violet-100 bg-violet-50/60 px-3 py-2">
                            <div className="relative h-12 w-12 overflow-hidden rounded-lg border bg-white shrink-0">
                              {item.asset?.fotoUrl ? (
                                <Image src={item.asset.fotoUrl} alt={item.asset.nome ?? 'Ativo'} fill className="object-cover" unoptimized />
                              ) : (
                                <div className="flex h-full items-center justify-center text-[10px] text-gray-400">SEM FOTO</div>
                              )}
                            </div>
                            <div className="min-w-0">
                              <p className="text-xs font-semibold text-violet-800">Ativo: {item.asset.nome}</p>
                              {item.asset.codigo && <p className="text-[11px] text-violet-700">Código: {item.asset.codigo}</p>}
                            </div>
                          </div>
                        )}
                      </div>
