...
import {
  ...
  PERIODICIDADE_COR,
  itensPorMultiplasPeriodicidades,
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          variant="outline"
          className={cn(
            "w-full justify-start font-normal rounded-xl border-gray-200 bg-white text-sm hover:bg-gray-50",
      ...
      <PopoverContent className="w-auto p-0" align="start">
        <CalendarPicker
          ...
          selected={date}
          onSelect={(d) => {
            if (d) { onChange(d.toISOString()); setOpen(false); }
          }}
          locale={ptBR}
          initialFocus
        />
...
function ModalCriarOS({
  mes,
  periodicidadesIniciais,
  onClose,
  onCriada,
}: {
  mes: Date;
  periodicidadesIniciais: string[];
  onClose: () => void;
  ...
  function togglePeriodicidade(per: string) {
    setPeriodicidadesSelecionadas((prev) =>
      prev.includes(per) ? prev.filter((p) => p !== per) : [...prev, per]
    );
  }

  const itensPreview = useMemo(
    () => itensPorMultiplasPeriodicidades(periodicidadesSelecionadas),
    [periodicidadesSelecionadas]
  );
  const prioridade = useMemo(
    () => (periodicidadesSelecionadas.some((p) => ["ANUAL", "SEMESTRAL"].includes(p)) ? "ALTA" : "MEDIA"),
    [periodicidadesSelecionadas]
  );

  async function criar() {
    if (periodicidadesSelecionadas.length === 0) {
      toast.error("Selecione ao menos uma periodicidade");
      return;
    }
    setLoading(true);
    try {
      const res = await fetch("/api/os", {
        ...
        body: JSON.stringify({
          ...
          dataFimProgramada: dataFim || undefined,
          subsistema: "Geral",
          prioridade,
        }),
      });
      ...
      if (!res.ok) {
        toast.error(json?.error?.formErrors?.[0] ?? json?.error ?? "Erro ao criar OS");
        return;
      }
        ...
        <div className="p-6 space-y-5">
          <div className="space-y-3">
            <label className="text-xs font-semibold text-gray-600 flex items-center gap-1.5">
              <CalendarRange className="w-3.5 h-3.5" /> Datas da visita
            </label>
            <div className="space-y-2">
              <div>
                <p className="text-xs text-gray-400 mb-1">Início</p>
                <DatePickerSimples value={dataInicio} onChange={setDataInicio} placeholder="Data de início" />
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">Fim (opcional)</p>
                <DatePickerSimples value={dataFim} onChange={setDataFim} placeholder="Data de fim" />
              </div>
            </div>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <span className={cn("text-[10px] font-normal", sel ? cor.text : "text-gray-400")}>
                      {itensPorMultiplasPeriodicidades([per]).length} itens
                    </span>
                  </button>
                );
              })}
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="button"
              onClick={criar}
              disabled={loading || periodicidadesSelecionadas.length === 0}
...
function buildColumns(year: number, viewMode: ViewMode): Column[] {
  ...
  return eachWeekOfInterval({ start, end }, { weekStartsOn: 0 }).map((weekStart, idx) => ({
    ...
    label: `S${idx + 1}`,
    sublabel: format(weekStart, "dd/MM"),
    start: startOfWeek(weekStart, { weekStartsOn: 0 }),
    end: endOfWeek(weekStart, { weekStartsOn: 0 }),
    dueAnchor: startOfWeek(weekStart, { weekStartsOn: 0 }),
    isCurrent:
      new Date() >= startOfWeek(weekStart, { weekStartsOn: 0 }) &&
      new Date() <= endOfWeek(weekStart, { weekStartsOn: 0 }),
  }));
}
...
function BadgeYear({ year }: { year: number }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white px-3 py-1.5 text-sm font-semibold text-gray-700">
      {year}
    </div>
  );
}
...
export function GanttOS({ ordens: ordensInicial }: { ordens: OSResumida[] }) {
  ...
  // Group by subsistema — returns array to keep order stable
  ...
  const handleCriada = useCallback(
    (novaOS: OSResumida) => setOrdensLocais((prev) => [...prev, novaOS]),
    []
  );

  // Fixed column widths
  const COL_ID = 48;       // px
  const COL_DESC = 260;    // px
  const COL_PERIOD = 90;   // px
  const COL_SLOT = 44;     // px per month/week column
  ...
  return (
    <>
      {modalState && (
        <ModalCriarOS
          mes={modalState.mes}
          periodicidadesIniciais={modalState.periodicidades}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-gray-50 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Ano completo com opção mensal ou semanal. A visita multi-periodicidade marca todas as linhas que ela cobre.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Button variant={viewMode === "mensal" ? "default" : "outline"} size="sm" onClick={() => setViewMode("mensal")}>
              Mensal
            </Button>
            <Button variant={viewMode === "semanal" ? "default" : "outline"} size="sm" onClick={() => setViewMode("semanal")}>
              Semanal
            </Button>
        ...
        <div className="overflow-x-auto">
          ...
          >
            <colgroup>
              ...
              {columns.map((col) => (
                <col key={col.key} style={{ width: COL_SLOT }} />
              ))}
            </colgroup>
            ...
            <thead>
              <tr className="border-b border-gray-100">
                {/* Sticky: ID */}
                <th
                  className="text-left px-2 py-2.5 text-xs font-semibold text-gray-500 bg-white"
                  style={{ position: "sticky", left: 0, zIndex: 20, width: COL_ID }}
                >
                  ID
                </th>
                {/* Sticky: Descrição */}
                <th
                  className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 bg-white"
                  style={{ position: "sticky", left: COL_ID, zIndex: 20, width: COL_DESC }}
                >
                  Descrição
                </th>
                {/* Período */}
                ...
                {/* Dynamic month/week columns */}
                {columns.map((col) => (
                  <th
                    key={col.key}
                    className={cn(
                      "text-center px-0 py-2.5 text-xs font-medium",
                      col.isCurrent ? "text-violet-700 bg-violet-50/70" : "text-gray-400 bg-white"
                    )}
                    style={{ width: COL_SLOT }}
                  >
                    <div className="flex flex-col items-center gap-0.5 leading-none">
                      <span className="truncate w-full text-center">{col.label}</span>
                      <span className={cn("text-[9px]", col.isCurrent ? "text-violet-500 font-bold" : "text-gray-300")}>
                        {col.sublabel}
                      </span>
                    </div>
                  </th>
            ...
            <tbody>
              {subsistemaGroups.map(([subsistema, rows]) => (
                <>
                  {/* Subsistema header row — spans all columns */}
                  ...
                  {rows.map(({ item, slots }) => {
                    ...
                    return (
                      <tr
                        key={item.id}
                        className="hover:bg-gray-50/60 transition-colors group border-b border-gray-50"
                      >
                        {/* Sticky: ID */}
                        <td
                          className="px-2 py-2 bg-white group-hover:bg-gray-50/60"
                        ...
                        {/* Sticky: Descrição */}
                        <td
                          className="px-3 py-2 bg-white group-hover:bg-gray-50/60"
                        ...
                        <td className="px-3 py-2" style={{ width: COL_PERIOD }}>
                          <span
                            className={cn(
                              "text-[10px] font-semibold px-1.5 py-0.5 rounded-md whitespace-nowrap inline-block",
                              cor?.bg ?? "bg-gray-100",
                              cor?.text ?? "text-gray-600"
                            )}
                          >
                            {item.periodicidade}
                          </span>
                          ...
                          return (
                            <td
                              ...
                              className={cn(
                                ...
                                slot.col.isCurrent && "bg-violet-50/30"
                              )}
                              style={{ width: COL_SLOT }}
                            >
                              ...
                              {slot.os && classif !== "nao_esperado" && (
                                <Link
                                  ...
                                  title={`${slot.os.numero} — ${slot.os.titulo}`}
                                  className={cn(
                                    "mx-auto w-8 h-6 rounded flex items-center justify-center",
                                    "hover:scale-110 hover:shadow-md transition-all cursor-pointer",
                                    "shadow-sm text-white text-[9px] font-bold relative",
                                    classif === "concluida"
                                      ? "bg-green-500"
                                      : classif === "atrasada"
                                      ? "bg-red-400"
                              ...
                              {!slot.os && classif !== "nao_esperado" && (
                                <button
                                  onClick={() =>
                                    setModalState({
                                      periodicidades: periodicidadesEsperadasNoMes(slot.col.dueAnchor),
                                      mes: slot.col.dueAnchor,
                                    })
                                  }
