      ...
      <Popover open={calOpen} onOpenChange={setCalOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="outline"
            className={cn(
              "flex-1 justify-start text-left font-normal rounded-xl border-gray-200 bg-white hover:bg-gray-50",
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const inputClass =
  "w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300 bg-white";
...
export function NovaOSButton() {
  ...
  const dataLimitePreview =
    form.dataEmissaoAxia && prazoHoras
      ? addHours(new Date(form.dataEmissaoAxia), prazoHoras)
      : null;
      ...
      const res = await fetch("/api/os", {
        ...
        body: JSON.stringify({
          ...
          dataProgramada: form.dataProgramada
            ? new Date(form.dataProgramada).toISOString()
            : undefined,
            ...
            <form onSubmit={submit} className="p-6 space-y-4">
              ...
              <Field label="Tipo de atividade" hint="Define o prazo SLA" required>
                <Select
                  ...
                  onValueChange={(v) => set("tipoAtividade", v)}
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Subsistema" required>
                  <Select
                    ...
                    onValueChange={(v) => set("subsistema", v)}
                  >
                    <SelectTrigger className="rounded-xl border-gray-200 focus:ring-violet-300 bg-white">
                ...
                <Field label="TAG do componente">
                  <Input
                    value={form.componenteTag}
                    onChange={(e) => set("componenteTag", e.target.value)}
                    className="rounded-xl border-gray-200 focus-visible:ring-violet-300"
                    placeholder="Ex: P01, G04, V202"
                  />
                </Field>
              </div>

              {/* Data emissão Axia + preview SLA */}
              <div className="rounded-xl bg-violet-50 border border-violet-100 p-4 space-y-3">
                <Field
                  label="Data e hora de emissão da OS — Axia (CONTRATANTE)"
                  required
                >
                  <DateTimePicker
                    value={form.dataEmissaoAxia}
                    onChange={(iso) => set("dataEmissaoAxia", iso)}
                    placeholder="Selecionar data e hora"
                    required
                  />
                </Field>
                ...
                {dataLimitePreview && (
                  <div className="bg-white/80 rounded-lg px-3 py-2 flex justify-between text-xs">
                    ...
                    <span className="font-semibold text-violet-700">
                      {format(dataLimitePreview, "dd/MM/yyyy 'às' HH:mm", {
                        locale: ptBR,
                      })}
                    </span>
                  </div>
              ...
              <Field label="Data programada para execução">
                <DateTimePicker
                  ...
                  onChange={(iso) => set("dataProgramada", iso)}
              ...
              <Field label="Motivo / Sintoma observado" required>
                <Textarea
                  ...
                  rows={2}
                  value={form.motivoOS}
                  onChange={(e) => set("motivoOS", e.target.value)}
                  className="rounded-xl border-gray-200 focus-visible:ring-violet-300 resize-none"
